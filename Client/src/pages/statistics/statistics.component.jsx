...
import { getWeekString, getMonthString, getYearString, checkIfDateIsBetweenWeek } from '../../utility';
...
export class Statistics extends Component {
	...
	setData = () => {
		...
		for (let i = 0; i < 5; i++) {
			let count = 0;
			Object.keys(this.props.pastWorkoutData).forEach((key) => {
				let isInsideWeek = checkIfDateIsBetweenWeek(this.props.pastWorkoutData[key].date, i - 4);
	...
	getNavString = (mode) => {
		switch (mode) {
			case this.modes.WEEK:
				return getWeekString(this.state.operator);
			case this.modes.MONTH:
				return getMonthString(this.state.operator);
			case this.modes.YEAR:
				return getYearString(this.state.operator);
			case this.modes.ALL:
				return 'Gesamt';
			default:
				return 'error';
		...
		return (
			<div className="statistics">
				...
				<div className="statistics__btn-container">
					<button onClick={() => this.setMode(WEEK)} className="btn btn--outline" id="">
						Woche
					</button>
					<button onClick={() => this.setMode(MONTH)} className="btn btn--outline" id="">
						Monat
					</button>
					<button onClick={() => this.setMode(YEAR)} className="btn btn--outline" id="">
						Jahr
					</button>
					<button onClick={() => this.setMode(ALL)} className="btn btn--outline" id="">
						Gesamt
					</button>
				</div>
				<div className="date-nav">
					<span
						onClick={() => this.setState({ operator: this.state.operator - 1 })}
						className="date-nav__arr-left"
					>
						&lang;
					</span>
					<span className="date-nav__content">{this.getNavString(this.state.displayMode)}</span>
					<span
						onClick={() => this.setState({ operator: this.state.operator + 1 })}
				...
				<div className="statistics__small-box">Sitzungen: {this.state.totalWorkouts}</div>
				<div className="statistics__small-box">Gesamtzeit: {this.state.totalTime}</div>
				<div className="statistics__small-box">&#216; Sitzungsdauer: {this.state.averageTime}</div>
				<div className="statistics__small-box">Saetze ges.: {this.state.totalSets}</div>
				<div className="statistics__big-box">
					...
					>
						...
						<Tooltip />
						{/* <Legend hide /> */}
						<Bar dataKey="workouts" fill="#56b4d3" />
					</BarChart>
				</div>
				<h3 className="h3-heading">Uebungen</h3>
				<div className="statistics__big-box">
					<LineChart width={325} height={300} data={data} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
						<XAxis dataKey="name" />
						<YAxis width={20} />
						<CartesianGrid strokeDasharray="3 3" />
						<Tooltip />
						{/* <Legend hide /> */}
						<Line type="monotone" dataKey="pv" stroke="#348f50" activeDot={{ r: 8 }} />
